#!/usr/bin/env python3
"""Assemble "An Afternoon at Kneaded.ai" — the SURPRISE cut (v3).

Act 1 reads as a FULLY HUMAN company (hub, debate, a normal team chat on a room
screen, a Kneaded.ai pitch slide, a friendly founder review). Then it TURNS
(agentic flows + an animated Kneaded.ai orchestration diagram) and REPLAYS the
beats revealed as AI via overlays (debate = AI group chat, presenter = AI deck),
landing on: everyone was an agent, except the founder. ElevenLabs narration,
burned mute-first captions. Output: review-room/output/v1.5.mp4 (+ v1.5.srt).
Run from repo root:  python3 scripts/review_room_surprise.py
"""
import os
import subprocess
import sys
...
ROOT = Path(__file__).resolve().parents[2]   # repo root (script archived at review-room/v1.5/)
RR = ROOT / "review-room"
STILLS = RR / "v1" / "owned" / "stills"
SAMPLES = RR / "raw"
ALT = RR / "v1.5" / "owned" / "altframes-codex"   # v1.5's own chosen alternates
...
SHOTS = [
    ...
    {"id": "03", "kind": "screenimg", "src": ALT / "chat_human.png",    "bg": ROOM, "dur": 6, "fin": False, "fout": False},
...
VO_LINES = [
    ...
    (20.3, "They take it to the founder. Who has notes. A lot of notes."),
    (24.3, "Except, there is no team."),
    (29.3, "Every role is an agent. One coordinator keeps them in sync."),
    (35.3, "The debate was a group chat."),
    (39.3, "The presenter was A.I."),
    (43.3, "Everyone here is an agent. Except the founder."),
    (47.3, "This is how Kneaded A.I. works."),
]
...
def screen_img(content, bg, dur, fin, fout, out):
    """Composite a static content image (chat / slide) onto a room screen."""
