#!/usr/bin/env python3
"""Assemble "An Afternoon at Kneaded.ai" — the SURPRISE cut.

Act 1 plays as a normal company (hub, debate, group chat, presentation), then
turns digital (agentic flows + the orchestration diagram), then replays the
beats revealed as AI (debate = group chat, presenter = AI deck). Narrated by
video-claw's $0 TTS with burned mute-first captions. Output:
review-room/surprise-v1.mp4 (+ .srt). Run from repo root:
...
ROOT = Path(__file__).resolve().parents[2]   # repo root (archived at review-room/v1.1/)
RR = ROOT / "review-room"
...
# Each beat is a dict. kinds: clip | screen | diagram | overlay | slate
SHOTS = [
    ...
    {"id": "03", "kind": "screen",  "src": GROUPCHAT, "bg": STILLS / "shot_05.png", "dur": 4, "ss": 0, "speed": 0.5, "fin": False, "fout": False},
    {"id": "04", "kind": "screen",  "src": DECK,      "bg": STILLS / "shot_05.png", "dur": 5, "ss": 2, "speed": 1.0, "fin": False, "fout": True},
    {"id": "05", "kind": "clip",    "src": SAMPLES / "sample4_agentic_flows.mp4", "dur": 5, "fin": True,  "fout": False},
    {"id": "06", "kind": "diagram", "src": DIAGRAM,   "dur": 6, "fin": False, "fout": True},
    {"id": "07", "kind": "overlay", "src": SAMPLES / "sample5_real_debate.mp4", "ov": GROUPCHAT, "speed": 0.5, "dur": 4, "fin": False, "fout": False},
    {"id": "08", "kind": "clip",    "src": DECK, "ss": 1, "dur": 4, "fin": False, "fout": True},
    {"id": "09", "kind": "slate",   "dur": 4, "fin": True, "fout": True},
]

# (desired_start, text) — offsets recomputed from measured WAV durations.
VO_LINES = [
    (1.0,  "An ordinary afternoon at Kneaded A.I."),
    (5.5,  "The team argues out what to build."),
    (10.5, "They hash it out in the group chat."),
    (14.5, "Someone presents the plan."),
    (19.5, "Except, there is no team."),
    (24.5, "Every session is an agent. One connector runs them all."),
    (30.3, "The debate was a group chat."),
    (34.3, "The presenter was A.I."),
    (38.5, "This is how Kneaded works."),
]
